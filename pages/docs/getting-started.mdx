...
<Callout emoji="ℹ️">
  If you experience issues logging in, it could be due to CORS. As the Form
  ...
  these two origins. You may need to install an extension in your browser that
  enables CORS to use the Form Builder properly.
</Callout>

![Login CORS error](/screens/cors-error.png)

### Create a new Form

Click the button named `Create a new form` in the navigation bar.
...
We'll start by providing a name for the new form we are building. We could type in the name into the `name` field of the schema. Alternatively, we could use the interactive schema builder to achieve this. Click the hamburger menu in the navbar to launch the builder. Type in `Test POC Patient Consent Form v1.0` as the name of your form. Click the green tick to the left when done to update the schema.
...
![Add a question to the form](/screens/consent-question.png)
...
![Concept lookup](/screens/concept-lookup.png)
...
![Select answers from concept lookup modal](/screens/concept-lookup-select-answers-modal.png)
...
![Question editor showing selected concept mappings and answers](/screens/concept-mappings-and-answers.png)
...
![Schema editor with Render Form button](/screens/rendered-schema.png)
