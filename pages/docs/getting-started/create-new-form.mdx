...
Open your maintained Form Builder deployment and sign in with an account issued by that server's administrator. This site no longer publishes a shared demo account. The server owner should rotate any account that was previously shared here.

![Login page](/screens/login.webp)

<Callout type="warning" emoji="⚠️">
  If the frontend and backend use different origins, configure allowed origins
  on the server or reverse proxy as part of your deployment. Do not disable or
  bypass browser CORS protections. See the archived [deployment
  guide](/docs/developer-guide/deploy-form-builder) for the legacy builder setup.
</Callout>
